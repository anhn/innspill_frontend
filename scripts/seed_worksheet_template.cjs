...
const MONGO_URI =
  "mongodb+srv://student_admin:stavanger25@clusterproject.5snil.mongodb.net/?retryWrites=true&w=majority&appName=ClusterProject";

if (!MONGO_URI) {
  console.error("❌ Error: MONGO_URI environment variable is not set.");
  console.error("\n📝 Please set MONGO_URI before running the script:");
  console.error(
    '   Windows PowerShell: $env:MONGO_URI="mongodb://127.0.0.1:27017/ai4edu_database"',
  );
  console.error(
    "   Windows CMD: set MONGO_URI=mongodb://127.0.0.1:27017/ai4edu_database",
  );
  console.error(
    '   Linux/Mac: export MONGO_URI="mongodb://127.0.0.1:27017/ai4edu_database"',
  );
  console.error("\n💡 Example MongoDB URIs:");
  console.error("   Local: mongodb://127.0.0.1:27017/ai4edu_database");
  console.error(
    "   Atlas: mongodb+srv://username:password@cluster.mongodb.net/ai4edu_database",
  );
  process.exit(1);
}
...
async function seed() {
  ...
  console.log(
    "   Or set MONGO_URI environment variable to your MongoDB connection string",
  );
  ...
  try {
    ...
    if (error.name === "MongooseServerSelectionError") {
      ...
      console.error(
        '      Windows: $env:MONGO_URI="mongodb://your-host:27017/ai4edu_database"',
      );
      console.error(
        '      Linux/Mac: export MONGO_URI="mongodb://your-host:27017/ai4edu_database"',
      );
      console.error(
        "   4. Use MongoDB Atlas or remote MongoDB connection string",
      );
    }
    throw error;
  }
  ...
  for (let i = 0; i < WORKSHEET_EXAMPLES.length; i++) {
    ...
    try {
      ...
      successCount++;
      console.log(
        `✓ [${i + 1}/${WORKSHEET_EXAMPLES.length}] Upserted: "${res.title}" (${res.educationLevel} - ${res.subjectArea})`,
      );
    } catch (error) {
      errorCount++;
      console.error(
        `❌ [${i + 1}/${WORKSHEET_EXAMPLES.length}] Error processing worksheet:`,
        error.message,
      );
      if (worksheet.title) {
